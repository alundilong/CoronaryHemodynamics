...
#
# Description
#     Define the ParaView installation that is present in the AddOns folder, in
#     case OpenFOAM's default version isn't available.
#
#------------------------------------------------------------------------------

if [ -n "$ParaView_DIR" ]
then
    if [ ! -d "$ParaView_DIR" -a -n "$AddOns_ParaView_DIR" ]
    then
        export ParaView_DIR=$AddOns_ParaView_DIR
    fi
fi
